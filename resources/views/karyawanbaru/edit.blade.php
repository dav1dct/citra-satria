...
@section('content')
    <h1 class="text-white text-center mb-6 h1 bg-primary p-3 rounded">Edit Status Karyawan</h1>
    ...
    <div class="w-full max-w-md mx-auto p-6 bg-gray-800 rounded shadow-md">
        <form action="{{ route('karyawanbaru.updateStatus', $karyawan->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" value="{{ $karyawan->pendidikan }}" class="block mt-1 w-full bg-gray-700 text-white" disabled>
            </div>

            <div class="mb-4">
                <x-input-label for="gender" :value="__('Jenis Kelamin')" />
                <input type="text" value="{{ $karyawan->gender }}" class="block mt-1 w-full bg-gray-700 text-white" disabled>
            </div>

            <div class="mb-4">
                <x-input-label for="alamat" :value="__('Alamat')" />
                <textarea id="alamat" class="block mt-1 w-full bg-gray-700 text-white" rows="3" readonly>{{ $karyawan->alamat }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status" class="block mt-1 w-full rounded border-gray-300 dark:bg-gray-900 dark:border-gray-700 dark:text-white" required>
                    @foreach(['Menunggu', 'Aktif', 'Tidak Aktif'] as $status)
